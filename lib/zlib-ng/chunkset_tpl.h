...
Z_INTERNAL __TARGET__ uint32_t CHUNKSIZE(void) {
...
Z_INTERNAL __TARGET__ uint8_t* CHUNKCOPY(uint8_t *out, uint8_t const *from, unsigned len) {
...
Z_INTERNAL __TARGET__ uint8_t* CHUNKUNROLL(uint8_t *out, unsigned *dist, unsigned *len) {
...
/* Copy DIST bytes from OUT - DIST into OUT + DIST * k, for 0 <= k < LEN/DIST.
   Return OUT + LEN. */
Z_INTERNAL __TARGET__ uint8_t* CHUNKMEMSET(uint8_t *out, unsigned dist, unsigned len) {
...
Z_INTERNAL __TARGET__ uint8_t* CHUNKMEMSET_SAFE(uint8_t *out, unsigned dist, unsigned len, unsigned left) {
