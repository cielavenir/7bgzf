 ...
 */
Z_INTERNAL __TARGET__ uint32_t UPDATE_HASH(deflate_state *const s, uint32_t h, uint32_t val) {
    (void)s;
    HASH_CALC(s, h, val);
 ...
 */
Z_INTERNAL __TARGET__ Pos QUICK_INSERT_STRING(deflate_state *const s, uint32_t str) {
    Pos head;
 ...
 */
Z_INTERNAL __TARGET__ void INSERT_STRING(deflate_state *const s, uint32_t str, uint32_t count) {
    uint8_t *strstart = s->window + str + HASH_CALC_OFFSET;
