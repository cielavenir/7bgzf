...
#define __TARGET__ __attribute__((target("avx512f,avx512dq,avx512vl,avx512bw,avx512vnni")))
#include "../../adler32_p.h"
...
Z_INTERNAL __TARGET__ uint32_t adler32_avx512_vnni(uint32_t adler, const uint8_t *src, size_t len) {
...
Z_INTERNAL __TARGET__ uint32_t adler32_fold_copy_avx512_vnni(uint32_t adler, uint8_t *dst, const uint8_t *src, size_t len) {
    if (src == NULL) return 1L;
    if (len == 0) return adler;
