...
static int _decompress(FILE *in, FILE *out, int nthreads){
	...
	int n=read_gz_header_generic(buf,2048,&offset,&len);
	...
	for(;i<total_block;i+=nthreads){
		...
		for(;i+j<total_block && j<nthreads;j++){
			...
			u32 pos=index<<0;
			if(pos>counter)fread(buf+BUFLEN-(pos-counter),1,pos-counter,in); //somehow compressed stream has dummy data between block. discard.
...
int _7razf(const int argc, const char **argv){
	...
	if(mode){
		...
		if(zlib){
			...
			ret=_compress(in,stdout,cryptopp,DEFLATE_CRYPTOPP,nthreads);
		}else if(kzip){
			fprintf(stderr,"(kzip)\n");
			ret=_compress(in,stdout,kzip,DEFLATE_KZIP,nthreads);
		}else if(store){
			fprintf(stderr,"(store)\n");
			ret=_compress(in,stdout,store,DEFLATE_STORE,nthreads);
		}
		fclose(in);
	}
#ifdef NOTIMEOFDAY
