...
int _7ciso(const int argc, const char **argv){
	...
	if(mode){
		...
		if(zlib){
			...
			ret=_compress(in,out,cryptopp,DEFLATE_CRYPTOPP,threshold,nthreads);
		}else if(kzip){
			fprintf(stderr,"(kzip)\n");
			ret=_compress(in,out,kzip,DEFLATE_KZIP,threshold,nthreads);
		}else if(store){
			fprintf(stderr,"(store)\n");
			ret=_compress(in,out,store,DEFLATE_STORE,threshold,nthreads);
		}
